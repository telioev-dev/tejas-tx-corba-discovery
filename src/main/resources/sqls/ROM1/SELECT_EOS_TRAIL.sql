       ...
       CASE
           ...
           WHEN sn.A_END_ME LIKE '%UME%'
               ...
               (SELECT NVL(p.ME_LABEL, 'N/F')
                FROM "tejasnetworks_route_rom 1" r
                         LEFT JOIN "tejasnetworks_ptp_rom 1" p
                ...
                WHERE r.snc_id = a.snc_id
                  ...
                  AND r.TUPLE_B =
                      (SELECT (MAX(TO_NUMBER(TUPLE_A)))
                       FROM "tejasnetworks_route_rom 1"
...
AS topology_type,
                		...
                		FROM
                			...
                			FROM
                		          FROM
                		              "tejasnetworks_route_rom 1" r1,
                		          "tejasnetworks_route_rom 1" r2,
                		          "tejasnetworks_ptp_rom 1" p1,
                		          "tejasnetworks_ptp_rom 1" p2
                			WHERE
                				r1.snc_id = r2.snc_id
                				AND r1.tuple_a = '1'
                				AND r2.tuple_a = '1'
                			...
                			UNION
                				...
                				WHERE
                					r1.snc_id = r2.snc_id
                					AND r1.tuple_a = (SELECT MIN(tuple_a) FROM "tejasnetworks_route_rom 1"   WHERE PATH_TYPE = 'PROTECTION' AND SNC_ID = r1.snc_id)
                					AND r2.tuple_a = (SELECT MIN(tuple_a) FROM "tejasnetworks_route_rom 1"  WHERE PATH_TYPE = 'PROTECTION' AND SNC_ID = r2.snc_id)
                		...
                		ORDER BY
                			...
                			TO_NUMBER(k.bval) DESC
                		         ...
                		         "tejasnetworks_topology_rom 1" t,
                		         "tejasnetworks_snc_rom 1" sn
WHERE
