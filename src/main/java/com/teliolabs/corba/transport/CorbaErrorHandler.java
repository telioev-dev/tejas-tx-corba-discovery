...
@Log4j2
public class CorbaErrorHandler {

    public static void handleProcessingFailureException(ProcessingFailureException pfe, String param)
            throws ProcessingFailureException {
        log.error(getError(pfe));
    }

    private static synchronized String getError(ProcessingFailureException _prf) {
        int errorCode = _prf.exceptionType.value();
        String str_error = "ProcessingFailureException: errorCode: " + errorCode + ", reason: " + _prf.errorReason + ", ";
        switch (errorCode) {
            case 0:
